...
class Augmentation(object):
    ...
    def rotate(self, im, rotation_param):
        ...
        im_new = cv2.warpAffine(np.array(im, dtype=np.float64), M, (nW, nH))
    ...
    def black_border_crop(self, im):
        ...
        x_thresh = np.where(np.max(np.max(np.asarray(im_new), axis=2), axis=0) > 10)[0]
        y_thresh = np.where(np.max(np.max(np.asarray(im_new), axis=2), axis=1) > 10)[0]
...
class OurAug(Augmentation):
    ...
    def process(self,img):
        ...
        if  self.params.get('rotation', False):
            rotate_params = np.random.randint(self.params['rotation_range'][0], self.params['rotation_range'][1])
        ...
        if self.params.get('contrast', False):
            contrast_param = np.random.uniform(self.params['contrast_range'][0], self.params['contrast_range'][1])
            im = self.contrast(im, contrast_param)

        if self.params.get('brightness', False):
            brightness_param = np.random.uniform(self.params['brightness_range'][0], self.params['brightness_range'][1])
            im = self.brightness(im, brightness_param)            

        if self.params.get('saturation', False):
            color_param = np.random.uniform(self.params['color_range'][0], self.params['color_range'][1])
            im = self.saturation(im, color_param)  

        if self.params.get('multiple_rgb', False):
            multiple_param = tuple(np.random.uniform(self.params['multiple_range'][0], self.params['multiple_range'][1],3))
